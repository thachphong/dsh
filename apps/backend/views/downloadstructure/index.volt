                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      {% for item in list %}
                          ...
                          <td>{{item['sort']}}</td> 
                          <td>
                            <span class="fa {%if item['ctg_flg'] == 0%}fa-square-o{%else%}fa-check-square{%endif%}" style="font-size: 16px;"></span>
                          </td>                     
                          <td>
                            <span class="fa {%if item['stop_flg'] == 1%}fa-square-o{%else%}fa-check-square{%endif%}" style="font-size: 16px;"></span>
                          </td>  
                          <td>
...
$(document).ready(function() {
    ...
    $(document).on('click','.btn_edit',function(event){
        var id = $(this).attr('id');
        id = id.replace("edit_","");      
        Pho_html_ajax('GET',"{{url.get('downloadstructure/edit/')}}"+ id ,null,function(html){
            Pho_modal(html,"Sửa cấu trúc download",700);
        });
    });
    $(document).off('click','#btn_new'); 
    $(document).on('click','#btn_new',function(event){
        Pho_html_ajax('POST',"{{url.get('downloadstructure/edit/0')}}" ,null,function(html){
    ...
    $(document).on('click','.btn_delete',function(event){
      ...
      Pho_message_confirm("Thông báo","Bạn có chắc chắn muốn xóa dòng này ?",function(){
        ...
        Pho_json_ajax('GET',"{{url.get('downloadstructure/delete/')}}" + id,null ,function(datas){
          if(datas.status == "OK"){
            Pho_direct("{{url.get('downloadstructure/index')}}");
          }else{
    ...
    $(document).on('click','#btn_save',function(event){
        ...
        Pho_json_ajax('POST',"{{url.get('downloadstructure/update')}}" ,arr,function(datas){
          if(datas.status =="OK"){
            Pho_modal_close("modal1");          
            Pho_direct("{{url.get('downloadstructure/index')}}");
          }else{
